...
/*var s = L.WMS.source("http://localhost:1111/geoserver/LocalPG/wms", {
    'transparent': true,
    'tiled': true
});
source.getLayer("MAKS_GRUP").addTo(map);
source.getLayer("AraziOrtusu_Corine").addTo(map);
layerControl.addOverlay(editableLayers, "AraziOrtusu_Corine");
*/
...
$(".leaflet-control-layers-list").prepend("<strong class='title'>Altlıklar</strong><br>");
$(".leaflet-control-layers-separator").after("<br><strong class='title'>Katmanlar</strong><br>");
...
layerControl.addOverlay(editableLayers, "Çizim Katmanı");
...
function initDrawSaveButton() {
  var saveFileElem = '<a class="leaflet-draw-edit-edit leaflet-disabled" href="javascript:ExportGeoJSON();" id="ExportGeoJSON" title="Kaydedilecek katman bulunamamıştır."><span class="sr-only">Katmanı dosyaya kaydet</span></a>';
...
function updateDrawTools()
{
  ...
  {
    $("#ExportGeoJSON").attr("title", "Eksport edilecek katman bulunamamıştır")
    $("#ExportGeoJSON").addClass("leaflet-disabled");
  }
  else
  {
    $("#ExportGeoJSON").attr("title", "Katmanı GeoJSON olarak export et")
